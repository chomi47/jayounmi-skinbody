...
const services = {
  prenatal: {
    eyebrow:'PRENATAL CARE', title:'산전관리', headline:'몸이 빠르게 변하는 시기,\n편안함을 먼저 생각합니다',
  ...
  postnatal: {
    eyebrow:'POSTNATAL CARE', title:'산후관리', headline:'출산 뒤 달라진 몸이\n천천히 리듬을 찾도록',
  ...
  'body-care': {
    eyebrow:'BODY & HAND CARE', title:'체형·약손관리', headline:'일상에 쌓인 긴장을 살피는\n섬세한 손의 균형',
...
function ServicePage({data}:{data:typeof services[keyof typeof services]}){
  return <main><Header/><section className="sub-hero"><p className="eyebrow">{data.eyebrow}</p><h1>{data.headline.split('\n').map((t,i)=><span key={t}>{t}{i===0&&<br/>}</span>)}</h1><p>{data.intro}</p></section>
  <section className="content-block two-column section-shell"><div><p className="eyebrow">FOR YOU</p><h2>이런 분께 필요합니다</h2></div><ul className="check-list">{data.needs.map(n=><li key={n}><Check size={18}/>{n}</li>)}</ul></section>
  <section className="soft-block"><div className="two-column section-shell"><div><p className="eyebrow">WHAT YOU FEEL</p><h2>고객이 겪는 불편</h2></div><p>{data.discomfort}</p></div></section>
  <section className="content-block two-column section-shell"><div><p className="eyebrow">OUR METHOD</p><h2>자연미의 관리 방법</h2></div><div><p>{data.method}</p><aside className="notice"><strong>관리 전 꼭 알려주세요</strong>{data.caution}</aside></div></section>
  <section className="dark-block"><div className="section-shell"><p className="eyebrow">CARE PROCESS</p><h2>관리 진행 순서</h2><ol className="process"><li><b>01</b><strong>예약 상담</strong><span>시기와 현재 상태를 확인합니다.</span></li><li><b>02</b><strong>당일 컨디션 확인</strong><span>불편한 부위와 주의사항을 듣습니다.</span></li><li><b>03</b><strong>맞춤 약손관리</strong><span>편안함을 확인하며 진행합니다.</span></li><li><b>04</b><strong>마무리 안내</strong><span>관리 후 유의사항을 설명합니다.</span></li></ol></div></section>
  <section className="content-block section-shell"><div className="section-title"><p className="eyebrow">TIME & FREQUENCY</p><h2>추천 횟수와 관리 시간</h2><p>개인의 상태와 목적에 따라 달라지므로 단정하지 않고 상담 후 안내합니다.</p></div><div className="info-cards"><article><Clock3/><h3>관리 시간</h3><p>희망 부위와 프로그램 구성에 따라 상담 후 안내</p></article><article><Sparkles/><h3>추천 횟수</h3><p>단회 상담 후 현재 상태와 목표에 맞춰 제안</p></article></div></section>
  <section className="soft-block testimonial-placeholder"><p className="eyebrow">REAL REVIEW</p><h2>실제 고객의 이야기를<br/>있는 그대로 전하겠습니다</h2><p>고객 동의를 받은 후기 원문과 관리 시점이 확인되면 이 영역에 게시합니다.</p></section>
  <section className="content-block section-shell faq"><div className="section-title"><p className="eyebrow">FAQ</p><h2>자주 묻는 질문</h2></div>{data.faq.map(([q,a])=><details key={q}><summary>{q}<span>+</span></summary><p>{a}</p></details>)}</section><Consultation/><Footer/></main>
}
